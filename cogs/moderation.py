...
class Moderation(commands.Cog):
    ...
    async def on_message(self, m):
        ...
        for letter in set(content):
            if content.count(letter) > SPAMTRESHOLD:
                ...
                embed.add_field(
                    name="Text", value=discord.utils.escape_markdown(m.content)
                )
                embed.set_footer(
                    text=f"Deleted at {datetime.datetime.utcnow().strftime('%a %b %d %H:%M:%S %Z')}"
                )
                await self.bot.logchannel.send(
                    f"Found spam in <#{m.channel.id}> by <@{m.author.id}>:", embed=embed
                )
    ...
    async def on_member_join(self, member):
        ...
        async with self.bot.pool.acquire() as con:
            mutedata = await con.fetch(
                "SELECT active FROM mutes where uid = $1", member.id
            )
            tempmutedata = await con.fetch(
                "SELECT endtime from tempmutes where uid  = $1", member.id
            )
            ...
            if mutedata:
                for mute in mutedata:
                    if mute["active"]:
                        ...
                        await self.bot.logchannel.send(
                            f"user {member.mention} ({member.id}) joined with active mute."
                        )
                        return
    ...
    async def kick(self, ctx, member: discord.Member, *, reason=None):
        ...
        embed.add_field(
            name="\u200b",
            value=f"If you have questions about this action, or would like "
        ...
        async with self.bot.pool.acquire() as con:
            try:
                result = await con.fetchrow(
                    "INSERT INTO kicks(uid, executor, timedate, reason) VALUES($1, $2, "
                    "CURRENT_TIMESTAMP(1), $3) RETURNING kickid",
                    member.id,
                    ctx.author.id,
                    reason,
                )
        ...
        if reason:
            embed = discord.Embed(
                title=f" ", description=f" ", color=discord.Color.green()
            )
    ...
    async def ban(
        self,
        ctx,
        member: typing.Union[discord.Member, discord.User, discord.Object],
        ...
        *,
        ...
        if isinstance(member, discord.Member):
            ...
            member = (
                ...
                else await self.bot.fetch_user(member.id)
            )
        ...
        async with self.bot.pool.acquire() as con:
            try:
                result = await con.fetchrow(
                    "INSERT INTO bans(uid, executor, timedate, reason) VALUES($1, $2, "
                    "CURRENT_TIMESTAMP(1), $3) RETURNING banid",
                    member.id,
                    ctx.author.id,
                    reason,
                )
    ...
    @commands.command(
        help="Bans the specified member for the specified reason for a specified time"
    )
    ...
    async def tempban(
        ...
        *,
        ...
        embed = discord.Embed(
            title=f"You have been temporary banned from {ctx.guild.name}. You have been banned until {endtime}.",
            color=discord.Color.green(),
        )
        if reason:
            embed.add_field(name="Reason:", value=f"{reason}", inline=False)
        ...
        async with self.bot.pool.acquire() as con:
            try:
                result = await con.fetchrow(
                    "INSERT INTO tempbans(uid, executor, timedate, endtime, reason) VALUES($1, $2, "
                    "CURRENT_TIMESTAMP(1), $3, $4) RETURNING banid",
                    member.id,
                    ctx.author.id,
                    endtime,
                    reason,
    ...
    async def warn(
        ...
        *,
        ...
        async with self.bot.pool.acquire() as con:
            try:
                result = await con.fetchrow(
                    "INSERT INTO warnings(uid, executor, timedate, reason) VALUES($1, "
                    "$2, CURRENT_TIMESTAMP(1), $3) RETURNING warnid",
                    member.id,
                    ctx.author.id,
                    reason,
                )
    ...
    async def mute(self, ctx, member: discord.Member, *, reason=None):
        ...
        async with self.bot.pool.acquire() as con:
            muteid = await con.fetchrow(
                "INSERT INTO mutes(uid, executor, timedate, reason, active) VALUES($1, $2, $3, $4, $5) RETURNING muteid",
                member.id,
                ctx.author.id,
        ...
        embed.add_field(
            ...
            value=f"If you have questions about this action, or would like "
            f"to appeal it. Please contact the staff team. "
            f"You were muted by {ctx.author.mention}",
            inline=False,
        )
        try:
            await member.send(embed=embed)
    ...
    @commands.command(
        help="mutes the specified member for the specified reason for a specified time"
    )
    @commands.has_permissions(manage_messages=True)
    async def tempmute(
        self,
        ...
        *,
        ...
        embed = discord.Embed(
            title=f"You have been temporary muted from {ctx.guild.name}. You have been muted until {endtime}.",
            color=discord.Color.green(),
        )
        if reason:
            embed.add_field(name="Reason:", value=f"{reason}", inline=False)
        ...
        async with self.bot.pool.acquire() as con:
            try:
                result = await con.fetchrow(
                    "INSERT INTO tempmutes(uid, executor, timedate, endtime, reason) VALUES($1, $2, "
                    "CURRENT_TIMESTAMP(1), $3, $4) RETURNING muteid",
                    member.id,
                    ctx.author.id,
                    endtime,
                    reason,
        ...
        embed = discord.Embed(title=f" ", description=f" ", color=discord.Color.green())
        embed.set_footer(text=f"Action performed by {ctx.author} | Case {muteid}")
    ...
    async def unmute(self, ctx, id: int, *, reason=None):
        ...
        try:
            async with self.bot.pool.acquire() as con:
                if temp:
                    userid = await con.fetchrow(
                        "UPDATE tempmutes SET endtime = $1 WHERE muteid = $2 RETURNING uid",
                        datetime.datetime.utcnow(),
                        id,
                    )
                    if not userid:
                    ...
                    userid = await con.fetchrow(
                        "UPDATE mutes SET active = $1 WHERE muteid = $2 RETURNING uid",
                        False,
                        id,
                    )
                    if not userid:
    ...
    async def infractions(
        self, ctx, member: typing.Union[discord.Member, discord.User, discord.Object]
    ):
        ...
        try:
            async with self.bot.pool.acquire() as con:
                warns = await con.fetch(
                    "SELECT * FROM warnings WHERE uid = $1", member.id
                )
                kicks = await con.fetch("SELECT * FROM kicks WHERE uid = $1", member.id)
                bans = await con.fetch("SELECT * FROM bans WHERE uid = $1", member.id)
                tempbans = await con.fetch(
                    "SELECT * FROM bans WHERE uid = $1", member.id
                )
            ...
            await ctx.send(
                f"Showing infractions for {member} (This could take some time)"
            )
            async with ctx.typing():
                # warnings
                for i, warn in enumerate(warns):
                    warnid = warn[0]
                    invoker = self.bot.get_user(warn[2]) or await self.bot.fetch_user(
                        warn[2]
                    )
                ...
                for i, kick in enumerate(kicks):
                    kickid = kick[0]
                    invoker = self.bot.get_user(kick[2]) or await self.bot.fetch_user(
                        kick[2]
                    )
                ...
                for i, ban in enumerate(bans):
                    banid = ban[0]
                    invoker = self.bot.get_user(ban[2]) or await self.bot.fetch_user(
                        ban[2]
                    )
    ...
    async def warncase(self, ctx, warnid: int):
        ...
        try:
            async with self.bot.pool.acquire() as con:
                case = await con.fetchrow(
                    "SELECT * FROM warnings WHERE warnid = $1", warnid
                )
    ...
    async def kickcase(self, ctx, kickid: int):
        ...
        try:
            async with self.bot.pool.acquire() as con:
                case = await con.fetchrow(
                    "SELECT * FROM kicks WHERE kickid = $1", kickid
                )
            embed = discord.Embed(color=0xB277DD)
            member = await self.bot.fetch_user(case[1])
            embed.set_author(name=str(member), icon_url=member.avatar_url)
